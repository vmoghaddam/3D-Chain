'use strict';
app.controller('materialProfileController', ['$scope', '$location', '$window', '$routeParams', '$rootScope', 'userService', 'authService', '$http', '$q', function ($scope, $location, $window, $routeParams, $rootScope, userService, authService, $http, $q) {
    ...
    $scope.bind = function (id, callback) {
        $scope.loadingVisible = true;
        userService.getMaterial(id).then(function (result) {
            $scope.loadingVisible = false;
            ...
            if (!$scope.profile.User.ImageUrl)
                $scope.profile.User.ImageUrl = '../../content/imgs/signup3.png';
