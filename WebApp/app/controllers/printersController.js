'use strict';
app.controller('printersController', ['$scope', '$location', '$window', '$routeParams', '$rootScope', 'userService', 'authService', '$http', '$q', function ($scope, $location, $window, $routeParams, $rootScope, userService, authService, $http, $q) {

    var $jq = jQuery.noConflict();
    $jq('body').on('click', '.link-profile-printer', function (e) {
        // do something
        e.preventDefault();
        var id = $jq(this).data('uid');
        if (authService.isAuthorized()) {
            $window.open('#!/printer/' + id);
        }
        else {
                ...
                store: {
                    ...
                    onLoaded: function (e) {
                        if ($scope.bindFilters) {
                            ...
                            //gool
                            userService.getMan().then(function (response) {

                                $scope.dg_man_ds = response;
                                userService.getAMTechnologies().then(function (response) {

                                    $scope.dg_tech_ds = response;
                                    userService.getAMProcesses().then(function (response) {

                                        $scope.dg_proc_ds = response;
                    ...
                    beforeSend: function (e) {
                        ...
                        var _techsIds = Enumerable.From(_techs).Select('$.AMTechnology').ToArray().join('_');
                        if (_techsIds)
                            e.params.techs = _techsIds;

                        var _mans = $scope.dg_man_instance.getSelectedRowsData();

                        var _mansIds = Enumerable.From(_mans).Select('$.Name').ToArray().join('_');
                        if (_mansIds)
                            e.params.mans = _mansIds;

                        var _proc = $scope.dg_proc_instance.getSelectedRowsData();
                        var _procIds = Enumerable.From(_proc).Select('$.AMProcess').ToArray().join('_');
                        if (_procIds)
                            e.params.astms = _procIds;
                        //AMProcess

                        var _gmat = $scope.dg_gmat_instance.getSelectedRowsData();
                        var _gmatIds = Enumerable.From(_gmat).Select('$.Title').ToArray().join('_');
                        if (_gmatIds)
                            e.params.gmats = _gmatIds;
                       
                        if ($scope.model)
                            e.params.Model = $scope.model;
                    },
                },
    ...
    $scope.dg_tech_columns = [
        ...
        {
            dataField: 'AMTechnology', caption: 'Title', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false,
    ...
    ////////////////////////////
    $scope.dg_proc_instance = null;
    $scope.dg_proc_ds = null;
    $scope.dg_proc_columns = [

        {
            dataField: 'AMProcess', caption: 'Title', allowResizing: true, alignment: 'left', dataType: 'string', allowEditing: false,
    ...
    $scope.dg_proc = {
        ...
        // dataSource: employees,
        columns: $scope.dg_proc_columns,
        onContentReady: function (e) {
            if (!$scope.dg_proc_instance)
                $scope.dg_proc_instance = e.component;
        },
        onSelectionChanged: function (e) {
        ...
        //},
        bindingOptions: {
            dataSource: 'dg_proc_ds',
            // height: 'dg_employees_height'
        }
    };
    ////////////////////////////
    $scope.dg_man_instance = null;
    $scope.dg_man_ds = null;
    $scope.dg_man_columns = [
    ...
    $scope.dg_man = {
       ...
       // paging: { pageSize: 20 },
        ...
        // dataSource: employees,
        columns: $scope.dg_man_columns,
        onContentReady: function (e) {
            if (!$scope.dg_man_instance)
                $scope.dg_man_instance = e.component;
        },
        onSelectionChanged: function (e) {
    ...
    $scope.dg_columns = [
       
        {
            ...
            cellTemplate: function (container, options) {
                var elem = "<div class='dg-cell-div' ><a class='dg-cell-link link-profile-printer' href='#' data-uid='" + options.data.Id + "' style='' target='_blank'   >" + options.data.Model + "</a></div>";
                $jq("<div>")
    ...
    $scope.dg_height = $jq(window).height() - 200;
    ...
    $scope.dg_columns_xs = [
        {
            ...
            cellTemplate: function (container, options) {
                ...
                var elem = "<div style='width:100%'>"
                    ...
                    + "<a class='dg-cell-link link-profile-printer' href='#' data-uid='" + options.data.Id + "' style='font-size:14px;'    >"
                    ...
                    + "<div class='dg-cell-div' ><a class='dg-cell-link link-profile-printer' href='#' data-uid='" + options.data.Id + "' style='font-size:15px;'    >" + options.data.Name + "</a></div>"
    ...
    $scope.$on('$viewContentLoaded', function () {
        ...
        for (i = 0; i < acc.length; i++) {
            acc[i].onclick = function () {
                ...
                $scope.$apply(function () {
                    ...
                    if (na > 0) {
                        $scope.dg_height = ($jq(window).height() - 200) + na * 355;
        ...
        //userService.getSMats().then(function (response) {

        //    $scope.dg_smat_ds = response;
